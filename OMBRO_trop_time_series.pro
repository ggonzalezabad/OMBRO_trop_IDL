; This script intends to create a long term time series of tropospheric
; BrO columns inferred from OMBRO data. OMBRO data has
...
; anomally are excluded, and contributions are weighted by area and fitting
...
; smoothing kernel
lonmin = -70.0 &  lonmax = -60.0
latmin = -25.0 &  latmax = -10.0

; Define geographic region of the Aral Sea
aral_lon_min = -67.5 & aral_lon_max = -66.5
aral_lat_min = -19.0 & aral_lat_max = -18.0

; Select other random region
rand_lon_min = -69.0 & rand_lon_max = -65.0
rand_lat_min = -24.0 & rand_lat_max = -22.0
...
OPEN_DEVICE, /PS, /COLOR, filename = 'Poopo_time_series_plots_0p2x0p2_SZA70_XT4-56_CF30.ps'
...
FOR ifile = 0, nfiles-1 do begin
   ...
   IF ( (ifile MOD 5.0) EQ 0.0) THEN BEGIN
      ...
      mask_lon = WHERE(lons GE -70 AND lons LE -64)
      mask_lat = WHERE(lats GE -21 AND lats LE -15)
      TVMAP, tropospheric_vcds[min(mask_lon):max(mask_lon),    $
                               min(mask_lat):max(mask_lat),    $
                               ifile],                         $
             lons[mask_lon], lats[mask_lat], /Sample, /Cbar,   $
             /coast, /countries, /continents, /noadvance,      $
             title = 'Tropospheric VCDs',                      $
             mindata = 0, nan_color=13, $;limit = [-21,-70,-15,-64], $
             maxdata = MAX(tropospheric_vcds[min(mask_lon):max(mask_lon), $
                                             min(mask_lat):max(mask_lat), $
                                             ifile],/nan)
...
cgplot, INDGEN(count)+1, Aral_final_data[0,mask], color = 1, xtitle ='Day #', $
        ytitle = 'Poopo # of data points in average', xrange = [0,count+2], xstyle = 1
cgplot, INDGEN(count)+1, Aral_final_data[1,mask], color = 1, xtitle ='Day #',  $
        ...
        title = 'Poopo Tropospheric BrO columns'
cgplot, INDGEN(count)+1, Aral_final_data[6,mask], color = 1, xtitle ='Day #',  $
        ...
        title = 'Poopo Stratospheric BrO columns'
...
cgplot, INDGEN(count)+1, Rand_final_data[1,mask], color = 1, xtitle ='Day #',  $
        ...
        yrange=[0.0e13,3e13], err_color=9, err_width=0,                     $
...
SAVE, stratospheric_vcds, tropospheric_vcds, Background_lat, $
      date_int, date_str, $
      filename = 'Poopo_strat_trop_vcds_0p2x0p2_SZA70_XT4-56_CF30_global.sav'
SAVE, aral_final_data, rand_final_data, date_int, date_str, $
      filename = 'Poopo_time_series_0p2x0p2_SZA70_XT4-56_CF30_global.sav'
